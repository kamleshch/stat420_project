...
lcf_data_raw$A049r = as.factor(lcf_data_raw$A049r) # number of persons in the household

lcf_data_raw$G018r = as.factor(lcf_data_raw$G018r) # number of adults in the household
...
# reject means residuals does not follow a normal distribution
get_ad_decision = function(model) {
  decide = unname(ad.test(resid(model))$p.value < 0.05)
  ifelse(decide, "Reject", "Fail to Reject")
}
...
The correlation and scatter plot shown in Figure 1 shows a strong correlation between `hhsize` and `noPersons`, and to a letter extent with `noAdults`, and `NoChildren`. This is confirmed in the correlation matrix of Figure 2. Thus `hhsize` will be used as a potential predictor for the model in place of `noPersons`, `noAdults`, and `NoChildren`.

Start with a simple additive model with all the predictors.

```{r}
add_model = lm(expenditure ~ income + hhsize + incomeSource + internet + economicHRP + SEC3Class + tenureType + sex_HRP, data = lcf_data)
```

Adjusted $R^2$ | RSME | LOOCV | Anderson-Darling Normality Test Result | Brausch-Pagan Test Result
:------: | :------: | :------: | :------: | :------: 
`r get_adj_r2(add_model)` | `r get_rmse(add_model)` | `r get_loocv_rmse(add_model)` | `r get_ad_decision(add_model)` | `r get_bp_decision(add_model)`
