...
Since `income` & `expenditure` are corrlated , lets create a model first
```{r}
model_income = lm(expenditure ~ income , data = lcf_data)
...
```{r warning=FALSE}
ggplot(lcf_data,aes(x = income , y = expenditure))+
  geom_point(col="darkorange") +
  geom_abline(intercept = coef(model_income)[1],slope = coef(model_income)[2],lwd=1)+
  annotate("text", x = 100, y=1100, label=paste("R2 = ",round(summary(model_income)$r.squared,3),sep = " "))
...
```{r}
model = lm(expenditure ~  income , data = lcf_data)
summary(model)
```
